...
5 = Refused Bequest: A subclass that inherits from a parent but ignores or barely uses the parent's
    interface. The inheritance relationship is semantically inappropriate — the child conceptually does
    not belong in the hierarchy. Look for: low override ratio, parent methods that make no sense in
    the child's domain, inheritance used purely for code reuse rather than type semantics.
6 = Shotgun Surgery: A class whose changes would force many small changes in other, semantically
    co-dependent classes. Distinguished from benign high fan-out (e.g., utility classes) by whether
    the coupled classes truly co-depend on the class's internal design. Look for: high outgoing coupling
    to domain-specific classes, scattered responsibilities where one logical change spans many classes.
7 = Inappropriate Intimacy: Two classes that excessively access each other's internal details
    (bidirectional coupling). Distinguished from legitimate collaborators (e.g., Order/OrderLine) by
    whether the bidirectional access is architecturally justified. Look for: classes reaching into each
    other's private state, mutual field/method access, tightly intertwined logic that blurs class boundaries.

Guidelines:
- Refused Bequest: The child overrides <30% of parent methods, or the parent's interface is semantically
  unrelated to the child's purpose.
- Shotgun Surgery: The class has outgoing calls to 5+ distinct domain classes (not utilities/loggers).
- Inappropriate Intimacy: Two classes have 3+ bidirectional method/field accesses between them.
